...
public sealed class ArbitrageOptions
{
    ...
    /// <summary>
    /// Порог фандинг-рейта в %. Если фандинг хотя бы по одной ноге строго ниже него,
    /// арбитражная сделка не открывается вовсе (пара открывается двумя ногами — 
    /// «провальной» ноги достаточно, чтобы пропустить сделку целиком).
    /// Значение по умолчанию: -1. Если фандинг по ноге неизвестен (биржа не отдаёт) —
    /// сделка разрешена (fail-open).
    /// </summary>
    [Range(-100, 100)]
    public decimal MinFundingRatePercent { get; set; } = -1m;
